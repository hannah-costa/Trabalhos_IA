...
void construirModelo(){
    for(int i = 0; i<SIZE; i++){
        for(int j = 0; j<SIZE; j++){
			if(i == 0 || i == SIZE-1){
				if(j == 0 || j == SIZE-1) modelo[i][j] = 3;
				else modelo[i][j] = 4;
			}
			else if(j == 0 || j == SIZE-1) modelo[i][j] = 5;
			else modelo[i][j] = 0;
        }
    }
}

void atualizarModelo(int esquerda, int direita, int cima, int baixo){
    modelo[posAgenteX][posAgenteY] = 0;
    modelo[posAgenteX-1][posAgenteY] = cima;
    modelo[posAgenteX+1][posAgenteY] = baixo;
    modelo[posAgenteX][posAgenteY-1] = esquerda;
    modelo[posAgenteX][posAgenteY+1] = direita;
}
...
int funcaoAgenteUtilidade(int esquerda, int direita, int cima, int baixo){
	//delay(500);

    int posNextPontoX = -1, posNextPontoY = -1;
    int posNextAsterX = -1, posNextAsterY = -1;
    int minDistPonto = 1000, tempDistPonto = 1000; // menor distância e distância temporária (ponto)
    int minDistAsterisco = 1000, tempDistAsterisco = 1000; // menor distância e distância temporária (asterisco)
    int encontrou = false;

	if(cima==2 ) return CIMA;
	if(direita==2) return DIREITA;
	if(esquerda==2) return ESQUERDA;
	if(baixo==2) return BAIXO;
    
	if (cima==1) return CIMA;
    if (direita==1) return DIREITA;
    if (esquerda==1) return ESQUERDA;
    if (baixo==1) return BAIXO;
    
    for (int i = 0; i < SIZE; i++){
        for (int j = 0; j < SIZE; j++){
            if (modelo[i][j] == 1 || modelo[i][j] == 2) {

                if (modelo[i][j] == 1) {
                    tempDistPonto = abs(posAgenteX-i) + abs(posAgenteY-j);
                    if (tempDistPonto < minDistPonto) {
                        minDistPonto = tempDistPonto;
                        posNextPontoX = i;
                        posNextPontoY = j;
                    }
                } else {
                    tempDistAsterisco = abs(posAgenteX-i) + abs(posAgenteY-j);
                    if (tempDistAsterisco < minDistAsterisco) {
                        minDistAsterisco = tempDistAsterisco;
                        posNextAsterX = i;
                        posNextAsterY = j;
                    }
                }
    ...
    if(encontrou) {
        if (minDistAsterisco < minDistPonto){
            if (posAgenteX > posNextAsterX) return CIMA;
            if (posAgenteY > posNextAsterY) return ESQUERDA;
            if (posAgenteY < posNextAsterY) return DIREITA;
            if (posAgenteX < posNextAsterX) return BAIXO;
        } else{
            if (posAgenteX > posNextPontoX) return CIMA;
            if (posAgenteY > posNextPontoY) return ESQUERDA;
            if (posAgenteY < posNextPontoY) return DIREITA;
            if (posAgenteX < posNextPontoX) return BAIXO;
        }
    }    
...
int main()
{
	...
	while(!verificarSucesso()){
        atualizarModelo(lerSensor(0), lerSensor(1), lerSensor(2), lerSensor(3));
        int acao = funcaoAgenteUtilidade(lerSensor(0), lerSensor(1), lerSensor(2), lerSensor(3));
