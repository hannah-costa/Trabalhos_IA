...
int funcaoAgenteObjetivo(int esquerda, int direita, int cima, int baixo){
	//delay(500);

    int posNextX = -1, posNextY = -1;
    int minDist = 1000, tempDist = 1000; // menor distância e distância temporária
    int encontrou = false;
    ...
    for (int i = 0; i < SIZE; i++){
        for (int j = 0; j < SIZE; j++){
            if (modelo[i][j] == 1 || modelo[i][j] == 2) {
                tempDist = abs(posAgenteX-i) + abs(posAgenteY-j);
                if (tempDist < minDist) {
                    minDist = tempDist;
                    posNextX = i;
                    posNextY = j;
    ...
    if(encontrou) {
        if (posAgenteX > posNextX) return CIMA;
        if (posAgenteY > posNextY) return ESQUERDA;
        if (posAgenteY < posNextY) return DIREITA;
        if (posAgenteX < posNextX) return BAIXO;
    }    
...
int main()
{
	...
	while(!verificarSucesso()){
        ...
        int acao = funcaoAgenteObjetivo(lerSensor(0), lerSensor(1), lerSensor(2), lerSensor(3));
